...
void sapxep_theo_ma (List &l);
int sua_sv_theo_ma(List &l, char ma[]);
int main(int argc, char *argv[])
{
	...
	do{
		...
		printf("5. Sap xep SV them ma\n");
		printf("6. Sap xep SV theo ma\n");
		...
		switch(chon){
			...
			case 4:{
				...
				if(xoa_sv_theo_ma(sv,ma)==0)
					printf("Da xoa !");
				else
			 		printf("Khong co ma nay!");
			}break;
			case 5: sapxep_theo_ma(sv); break;
			case 6:{
				char ma [8];
				printf("Nhap ma sv can sua:");scanf("%s",&ma);
				if(sua_sv_theo_ma(sv,ma)==1)
					printf("da sua");
				else
					printf("khong co sv nay!");
			}
		}
	}while(chon<10);
	NhapDSSV(sv);
	...
	for(Node *p=l.first;p->link!=NULL; p=p->link)
		for(Node*q=p->link;q!=NULL;q=q->link)
			if(strcmp(p->data.masv,q->data.masv)>0){
				//tang dan
				Sinhvien x=p->data;p->data=q->data;q->data=x;
			}
}

// sua sv them ma (giu nguyen ma sv)
int sua_sv_theo_ma(List &l, char ma[]){
	Node*p=l.first;
